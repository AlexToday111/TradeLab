...
export function AppShell({ children }: { children: ReactNode }) {
  return (
    <div className="relative isolate h-dvh w-full overflow-hidden text-foreground">
      ...
      <div className="mx-auto flex h-full w-full max-w-[1640px] flex-col px-2 py-3 md:px-4 md:py-4">
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[30px] border border-[hsl(var(--tl-border-1)/0.72)] bg-[linear-gradient(180deg,hsl(var(--tl-bg-1)/0.96),hsl(var(--tl-bg-0)/0.98))] shadow-[inset_0_1px_0_hsl(var(--tl-glass-highlight)/0.08),0_30px_90px_rgba(0,0,0,0.18)]">
          ...
          <Topbar />
          <main className="relative min-h-0 flex-1 overflow-hidden bg-[linear-gradient(180deg,hsl(var(--tl-bg-1)/0.94)_0%,hsl(var(--tl-bg-0)/0.96)_100%)]">
            ...
            <div className="relative h-full overflow-y-auto p-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:p-5 lg:p-6">{children}</div>
          </main>
        </div>
      </div>
    </div>
